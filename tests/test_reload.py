...
time_tic2 = {"num_updates": 20, "num_epochs": 1}
stats_tic2 = {"train_loss": 0.2, "test_loss": 0.1}
time_tic3 = {"num_updates": 30, "num_epochs": 1}
stats_tic3 = {"train_loss": 0.223, "test_loss": 0.097}
time_tic4 = {"num_updates": 40, "num_epochs": 1}
...
def test_reload():
    ...
    assert (collections.Counter(relog.model_log.top_k_ckpt_list)
            == collections.Counter(['reload_dir/models/top_k/top_k_no_seed_provided_top_0.pt',
                                    'reload_dir/models/top_k/top_k_no_seed_provided_top_1.pt']))
    assert (collections.Counter(relog.model_log.top_k_storage_time)
            == collections.Counter([20, 30]))
    assert np.allclose(relog.model_log.top_k_performance, [0.097, 0.1])
    assert (collections.Counter(relog.model_log.every_k_storage_time)
            == collections.Counter([20]))
    assert (collections.Counter(relog.model_log.every_k_ckpt_list)
            == collections.Counter(["reload_dir/models/every_k/every_k_no_seed_provided_k_2.pt"]))

    # Check correctness of figure paths
    assert (collections.Counter(relog.figure_log.fig_storage_paths)
            == collections.Counter(['reload_dir/figures/fig_1_no_seed_provided.png',
    ...
    assert (collections.Counter(relog.extra_log.extra_storage_paths)
            == collections.Counter(['reload_dir/extra/extra_1_no_seed_provided.pkl',
    ...
    assert np.allclose(relog.stats_log.stats_to_track['test_loss'],
                       np.array([0.1235, 0.1, 0.097]))
    assert np.allclose(relog.stats_log.clock_to_track['num_updates'],
                       np.array([10, 20, 30]))
    ...
    # Check correctness of reloaded statistics
    assert np.allclose(np.array(relog.stats_log.stats_to_track['test_loss'].values.tolist()),
                       np.array([0.1235, 0.1, 0.097, 0.085]))
    assert np.allclose(np.array(relog.stats_log.clock_to_track['num_updates'].values.tolist()),
